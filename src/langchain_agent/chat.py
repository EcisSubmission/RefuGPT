...
Return only the integer 0 or 1 based on the primary intent of the message.
    ...
    def run_conversational_retrieval_chain(self, message: str, chat_history: list, message_language: str, type: str = "official", k: int = 5) -> dict:
        if type == "official":
            ...
            prompt_template = """You are assisting with information retrieval for Ukrainian refugees. Given the context below, answer the subsequent question. The answer should be eloquent, easily understandable, if helpful provide step by step guide. Please solely provide content that answers the given question. Cite the "source URL" you rely on from given the context at the complete end of your answer. You can also cite multiple URLs if you like. Please return NO_INFORMATION if the question cannot be answered with the given context.
{context}
Question: {question}
